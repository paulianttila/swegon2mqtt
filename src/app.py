...
class MyApp:
    ...
    def init(self, callbacks: Callbacks) -> None:
        ...
        self.received_messages_errors_metric = Counter('received_message_errors', '', registry=self.metrics_registry)
        self.exit = False
    ...
    def parse_message(self, msg):
        ...
        self.logger.debug('unpackStr=%s, msg len=%s, msg type=%s, data=%s',
            unpackStr, len(msg), type(msg), binascii.hexlify(msg).upper())

        firstByte, destinationAddress, sourceAddress, dataLen, msgType, msgData = struct.unpack(unpackStr, msg)
        self.logger.debug('firstByte=%02X, destinationAddress=%02X, sourceAddress=%02X, dataLen=%02X, msgType=%02X, msgData=%s', 
            firstByte, destinationAddress, sourceAddress, dataLen, msgType, binascii.hexlify(msgData).upper())
    ...
    def parse_msg21(self, msg):
        self.logger.debug('Parse message 0x21, data: %s' % binascii.hexlify(msg).upper())
        data = bytearray(msg)
    ...
    def parse_msg71(self, msg):
        self.logger.debug('Parse message 0x71, data: %s' % binascii.hexlify(msg).upper())
        data = bytearray(msg)
        ...
        if (extractTemp - outdoorTemp) != 0:
            calcSupplyEfficiency = int( ( float(supplyTemp - outdoorTemp) / float(extractTemp - outdoorTemp) ) * 100)
            calcExtractEfficiency = int( ( float(extractTemp - exhaustTemp) / float(extractTemp - outdoorTemp) ) * 100 )
    ...
    def parse_msg73(self, msg):
        self.logger.debug('Parse message 0x73, data: %s' % binascii.hexlify(msg).upper())
        data = bytearray(msg)
    ...
    def handle_data(self, data):
        self.logger.debug('Received data (type=%s): %s', type(data), binascii.hexlify(data).upper())
        d = bytearray(data)
    ...
    def handle_message(self, data):
        if len(data) > 10 and data[0] == 0xCC:
            ...
            if calcCrc == crcFromMsg:
                self.logger.debug('CRC OK, Message: %s, 0x%04X (crcFromMsg) == 0x%04X (calcCrc)',
                    binascii.hexlify(msg).upper(), crcFromMsg, calcCrc)
                if self.analyzer_mode:
                    self.analyze_message(msg)
                self.parse_message(msg)
            else:
                if self.analyzer_mode:
                    self.logger.error('CRC FAILURE, PDU: %s, Message: %s, 0x%04X (crcFromMsg) !=0x%04X (calcCrc)',
                        binascii.hexlify(data).upper(), binascii.hexlify(msg).upper(), crcFromMsg, calcCrc)
                else:
                    self.logger.debug('CRC FAILURE, PDU: %s, Message: %s, 0x%04X (crcFromMsg) !=0x%04X (calcCrc)',
                        binascii.hexlify(data).upper(), binascii.hexlify(msg).upper(), crcFromMsg, calcCrc)
        else:
            if self.analyzer_mode:
                self.logger.error('Invalid message received, PDU: %s', binascii.hexlify(data).upper())
            else:
                self.logger.debug('Invalid message received, PDU: %s', binascii.hexlify(data).upper())
    ...
    def analyze_message(self, msg):
        ...
        self.logger.error('Diff (%02X: %02X->%02X): %s',
            msgType, sourceAddress, destinationAddress, self.inline_diff(p, n))
    ...
    def inline_diff(self, a, b):
        ...
        def process_tag(tag, i1, i2, j1, j2):
            ...
            NORMAL = CSI + "0;0m"
            if tag == 'replace':
                return GREEN + '{' + str(matcher.a[i1:i2]) + ' -> ' + str(matcher.b[j1:j2]) + '}' + NORMAL
            elif tag == 'delete':
